...
class MVCTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testInit()
    {
        ...
        $mvc = new MVC('http://localhost/main/index/id?a=1&b=c#123', true);
        $this->assertEquals('main', $mvc->controller);
        $this->assertEquals('index', $mvc->action);
    ...
    {
        $mvc = new MVC("http://localhost/main/test/main123", true);
        $mvc->call();
    }
    ...
    {
        $mvc = new MVC("http://localhost/main/main", true);
        $this->assertEquals('main', $mvc->controller);
        $this->assertEquals('main', $mvc->action);
        $mvc->call();

        $mvc = new MVC("http://localhost/main1/index", true);
        $this->assertEquals('main1', $mvc->controller);
        $this->assertEquals('index', $mvc->action);
        $mvc->call();

        $mvc = new MVC("http://localhost/test/main1", true);
